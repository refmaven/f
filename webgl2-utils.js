 ...
 */
export function getGLContext(canvasID) {
 ...
 */
export function setIndexBuffer(gl, data, usage = gl.STATIC_DRAW) {
  const buffer = gl.createBuffer();
 ...
 */
export function setAttribute(gl, program, buffer, attributeName, size, type = gl.FLOAT, stride = 0, offset = 0) {
 ...
 */
export function setUniform(gl, program, uniformName, type, value) {
 ...
 */
export function draw(gl, mode, count, type = gl.UNSIGNED_SHORT) {
...
/**
 * Helper function to preprocess GLSL shader code, adding the version directive and expanding the main function shorthand.
 * @param {WebGL2RenderingContext} gl - The WebGL2 rendering context.
...
export function createShader(gl, type, source) {
  ...
  // Preprocess the source to automatically expand main function shorthand
...
/**
 * Creates and links a shader program using the helper `createShader`.
 * @param {WebGL2RenderingContext} gl - The WebGL2 rendering context.
 * @param {string} vertexSource - GLSL source code for the vertex shader.
 * @param {string} fragmentSource - GLSL source code for the fragment shader.
 * @returns {WebGLProgram|null} The created shader program, or null if there was an error.
 */
export function createProgramWithHelpers(gl, vertexSource, fragmentSource) {
...
/**
 * Helper function to handle the rendering pipeline.
 * It resizes the canvas, clears the screen, and draws the object.
 *
 ...
 * @param {WebGLBuffer} indexBuffer - The buffer containing index data.
 * @param {number} indexCount - The number of indices to draw.
 * @param {number} mode - The drawing mode (e.g., gl.TRIANGLES, gl.LINES, etc.).
 */
export function render(gl, program, vertexBuffer, indexBuffer, indexCount, mode = gl.TRIANGLES) {
    // Resize the canvas to fit the display size
    resizeCanvas(gl);

    // Clear the canvas to prepare for rendering
    clear(gl, 0.1, 0.1, 0.1, 1.0);  // Set the clear color to a dark gray

    // Use the provided shader program
    gl.useProgram(program);

    // Set the vertex buffer (attributes)
    setAttribute(gl, program, vertexBuffer, 'a_position', 2);  // Assuming a 2D position attribute

    // Bind the index buffer
    gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, indexBuffer);

    // Draw the elements (or vertices)
    draw(gl, mode, indexCount);

    // Request the next frame to keep rendering (if needed)
    requestAnimationFrame(() => render(gl, program, vertexBuffer, indexBuffer, indexCount, mode));
}

/**
 * Helper function to render multiple shapes.
 * Each shape will have its own vertex and index data.
 *
 * @param {WebGL2RenderingContext} gl - The WebGL context.
 * @param {WebGLProgram} program - The shader program to use for drawing.
 * @param {Array} shapes - An array of objects containing vertex and index buffers, along with the count.
 */
export function renderMultipleShapes(gl, program, shapes) {
    // Resize the canvas to fit the display size
    resizeCanvas(gl);
    ...
    for (let shape of shapes) {
        // Set the vertex buffer (attributes)
        setAttribute(gl, program, shape.vertexBuffer, 'a_position', 2);  // Assuming a 2D position attribute

        // Bind the index buffer
        gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, shape.indexBuffer);

        // Draw the elements (or vertices)
        draw(gl, gl.TRIANGLES, shape.indexCount);
    }

    // Request the next frame to keep rendering (if needed)
    requestAnimationFrame(() => renderMultipleShapes(gl, program, shapes));
}